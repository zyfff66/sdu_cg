  ...
  if (args->animate) {
      if (args->num < 0) {
          for (int i = 0; i < 10; i++) {
              if (cloth)
              {
                  if (args->animateType == AnimateType::Animate)
                    cloth->Animate();
                  else if (args->animateType == AnimateType::Runge_Kutta)
                    cloth->Runge_Kutta();
                  else if (args->animateType == AnimateType::AdaptiveTimestep)
                    cloth->AdaptiveTimestep();
              }
          }
      }
      else {
          ...
          for (int i = 0; i < 10; i++) {
              ...
              else if (num == args->num) {
                  end_time = clock();
                  double endtime = (double)(end_time - start_time) / CLOCKS_PER_SEC;
                  //std::cout << "1000 rounds, total time:" << endtime << std::endl;		//s为单位
                  std::cout << "Total iterations: " << args->num << ". Total time : " << endtime * 1000 << "ms, " << endtime << "s" << std::endl;	//ms为单位
                  num++;
              }
          }
      }
